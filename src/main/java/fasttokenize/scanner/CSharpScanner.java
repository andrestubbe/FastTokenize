...
/**
 * High-performance lexical scanner for C# (.NET).
 */
public class CSharpScanner implements CodeScanner {
    ...
    private static final Set<String> KEYWORDS = Set.of(
        ...
        "async", "await", "get", "set", "init", "record", "var", "yield"
    );

    private static final Set<String> TYPES = Set.of(
        "String", "Object", "Int32", "Int64", "Double", "Single", "Boolean",
        "DateTime", "Task", "List", "Dictionary", "IEnumerable", "Console"
    );
    ...
    public List<Token> scan(CharSequence source) {
        ...
        while (i < len) {
            ...
            // VERBATIM & INTERPOLATED STRINGS (@"..." or $"..." or $@"...")
            if (c == '@' || c == '$') {
                int start = i;
                if (c == '$' && i + 1 < len && source.charAt(i + 1) == '@') i += 2;
                else if (c == '@' && i + 1 < len && source.charAt(i + 1) == '$') i += 2;
                else i++;
            ...
            // ATTRIBUTES ([Serializable], [HttpGet])
